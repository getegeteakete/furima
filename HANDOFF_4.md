...
### ★今回NEW★ ⓪-b Supabase で 0004_products_stock.sql を実行（商品在庫カラム）
- `supabase/migrations/0004_products_stock.sql` を SQL Editor で実行（冪等）
- products に stock 列を追加。未実行でも商品表示・追加は動くが在庫編集が保存されない
...
2. 商品画像・プロフィール画像も Storage へ（chat-images と同パターン）
   ...
   - 商品管理UIに画像アップロードを追加（現状はアイコン選択のみ）
...
5. 在庫数の自動減算（購入確定で stock を減らす・運営は決済を持たない方針内で）
